...
class camera
{
	public:
		...
		__device__ camera(vec3 lookfrom, vec3 lookat, vec3 vup, float vfov, float aspect, float aperture, float focusDist, float t0, float t1)
		{
			this->t0 = t0;
			this->t1 = t1;
		...
		__device__ ray getRay(float s, float t, curandState *randState)
		{
			...
			vec3 rd = lensRadius * randomInUnitDisk(randState);
			vec3 offset = u * rd.x() + v * rd.y();

			float time = t0 + curand_uniform(randState) * (t1 - t0);
